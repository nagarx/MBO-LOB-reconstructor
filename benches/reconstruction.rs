...
fn bench_analytics(c: &mut Criterion) {
    ...
    group.bench_function("microprice", |b| b.iter(|| black_box(state.microprice())));

    group.bench_function("spread_bps", |b| b.iter(|| black_box(state.spread_bps())));

    group.bench_function("depth_imbalance", |b| {
        b.iter(|| black_box(state.depth_imbalance()))
    });

    group.bench_function("vwap_bid", |b| b.iter(|| black_box(state.vwap_bid(5))));
