...
use mbo_lob_reconstructor::{
    LobReconstructor, MboMessage, Action, Side, LobState,
    LobConfig, CrossedQuotePolicy,
    DayStats, NormalizationParams,
    DepthStats, MarketImpact, LiquidityMetrics,
};
...
fn demo_enriched_analytics(state: &LobState) {
    ...
    if let Some(imbalance) = state.depth_imbalance() {
        let direction = if imbalance > 0.0 { "BUY pressure" } else { "SELL pressure" };
        println!("     Depth Imbalance:  {:.4} ({})", imbalance, direction);
    }
...
fn demo_depth_stats(state: &LobState) {
    ...
    println!("     VWAP:             ${:.4}", bid_stats.weighted_avg_price);
    if let (Some(best), Some(worst)) = (bid_stats.best_price, bid_stats.worst_price) {
        println!("     Best Price:       ${:.4}", best);
        println!("     Worst Price:      ${:.4}", worst);
    }
    println!("     Price Range:      ${:.4}", bid_stats.price_range);
    println!("     Concentration:    {:.2}%", bid_stats.concentration_ratio * 100.0);
    ...
    println!("     VWAP:             ${:.4}", ask_stats.weighted_avg_price);
    println!("     Concentration:    {:.2}%", ask_stats.concentration_ratio * 100.0);
...
fn demo_market_impact(state: &LobState) {
    ...
    println!("     Slippage:         ${:.6} ({:.2} bps)", small_buy.slippage, small_buy.slippage_bps);
    println!("     Levels Consumed:  {}", small_buy.levels_consumed);
    println!("     Filled:           {} / {}", small_buy.filled_quantity, small_buy.requested_quantity);
    ...
    println!("     Slippage:         ${:.6} ({:.2} bps)", medium_buy.slippage, medium_buy.slippage_bps);
    ...
    println!("     Available:        {}", large_buy.total_quantity_available);
    println!("     Filled:           {}", large_buy.filled_quantity);
    println!("     Unfilled:         {}", large_buy.unfilled_quantity);
    println!("     Fill Ratio:       {:.1}%", large_buy.fill_ratio() * 100.0);
    ...
    println!("     Slippage:         ${:.6} ({:.2} bps)", sell.slippage, sell.slippage_bps);
...
fn demo_liquidity_metrics(state: &LobState) {
    ...
    println!("     Spread:           ${:.4} ({:.2} bps)", metrics.spread, metrics.spread_bps);
...
fn demo_day_stats() {
    ...
    println!("     Original:         ${:.4}", test_price);
    println!("     Normalized:       {:.4} (z-score)", normalized);
